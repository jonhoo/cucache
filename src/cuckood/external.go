...
func New(esize uint64) Cuckoo {
	if esize == 0 {
		esize = 1 << 16
	}
	return Cuckoo{sync.RWMutex{}, unsafe.Pointer(create(esize)), esize}
}
...
func (c *Cuckoo) op(key []byte, upd Memop) MemopRes {
	...
	if res.T == SERVER_ERROR && h == MAX_HASHES {
		...
		if c.get() == m {
			nsize := c.size << 1
			fmt.Fprintln(os.Stderr, "insert failed on key", string(key), ", so growing hashtable to", nsize)
